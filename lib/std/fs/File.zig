...
pub const Writer = struct {
    ...
    pub fn drain(io_w: *std.Io.Writer, data: []const []const u8, splat: usize) std.Io.Writer.Error!usize {
        ...
        const can_use_writev = !is_windows and
        ...
        if (!can_use_writev) {
            ...
            if (buffered.len != 0) {
                const n = posix.pwrite(handle, buffered, pos) catch |err| {
                    w.err = err;
            ...
            for (data[0 .. data.len - 1]) |buf| {
                ...
                const n = posix.pwrite(handle, buf, pos) catch |err| {
                    w.err = err;
                    return error.WriteFailed;
                };
                w.pos += n;
            ...
            const n = posix.pwrite(handle, splat_buffer, pos) catch |err| {
                w.err = err;
                return error.WriteFailed;
            };
            w.pos += n;
